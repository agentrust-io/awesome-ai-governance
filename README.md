...
- [Kepil](https://github.com/oleg-vdv/kepil) - AGPL-3.0 accountability layer that issues a passport per agent version (purpose, boundaries, self-declared risk and autonomy class) and a per-job mandate with allowed actions, systems, spending limits and expiry. Every action is checked against the mandate before a model is called, fail-closed. The passport constructor refuses the autonomy class in which human cancellation is impossible. Python stdlib only, no dependencies.
...
- [agent-trace](https://github.com/oleg-vdv/agent-trace) - MIT verifier for hash-chained agent action journals, deliberately written in a different language from the writer (Python writes, TypeScript verifies) so that a log is not checked only by its author's own tool. Reports the index of the record that changed, not just that the file differs; number canonicalisation follows a subset of RFC 8785.
