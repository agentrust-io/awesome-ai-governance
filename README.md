...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - On-chain trust scoring and reputation verification for AI agent wallets on Solana. MCP server (`score_agent`, `preflight_check`, `get_trust_receipt`) that queries verifiable on-chain wallet history to authorize agent-to-agent actions before execution. Complements DID-based identity schemes with behavioral reputation data; issues signed, x402-gated trust receipts for immutable audit trails.
