...
- [AI Control Tower](https://github.com/kironovlaziz-del/AI-tower#readme) - Self-hosted AI governance platform with policy approvals, request auditing, prompt masking with optional named-entity recognition, and a registry of reported shadow-AI sightings. Apache-2.0.
...
- [OrcaPromptVault](https://github.com/Continuum-AI-Corp/OrcaPromptVault) - The pre-action half of the record: what the agent was told and which tools it was handed, before it acted. 110 system prompts and 16 tool-schema files from 35 shipping products, 35 recorded off the wire by a local proxy with the command that reproduces each, so the instruction set of a given version can be pinned and re-derived rather than asserted. Claude Code declares 35 tools interactively and 29 under `claude -p` on the same version. AGPL-3.0.
