...
- [Forge Agent Gate](https://github.com/forgeorbital/forge-agent-gate) - Runtime gate that runs a deterministic policy check before an agent takes an irreversible action (payments, refunds, deletes), then emits a cryptographically signed, tamper-evident decision record. Open-source (MIT), ships as a TypeScript package and MCP server.
