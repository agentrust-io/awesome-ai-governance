...
- [agent-evidence-admission](https://github.com/astrogilda/agent-evidence-admission) - Apache-2.0 admission rails that allow or deny a Kubernetes workload on the in-toto agent-execution evidence attached to its image: a Rego policy for OPA plus Kyverno (JMESPath and CEL) and sigstore policy-controller (CUE) equivalents, each declaring per obligation what it enforces, approximates, or cannot reach, measured against the agent-evidence-vectors corpus at v0.10.1 in CI.
...
- [agent-evidence-vectors](https://github.com/astrogilda/agent-evidence-vectors) - Conformance corpus and reference verifier for records of what an agent did: 461 vectors across eight corpora at v0.10.1, covering the in-toto adversarial-execution-evidence and ai-agent-action predicates, SCITT/COSE carriage (RFC 9943 and RFC 9942), an anchor stream, an artifact-binding contract, a cross-run record contract and ACI deployments. One Go verifier judges every corpus and recomputes each outcome from the bytes the statement carries, so a consumer can reject a producer whose own verdict is false. Apache-2.0; archived at Zenodo, DOI 10.5281/zenodo.22758687.
...
- [agent-evidence-vocabulary](https://github.com/astrogilda/agent-evidence-vocabulary) - Closed, versioned vocabulary (CC0) for the terms an adversarial-execution evidence claim is written in: what an executed artifact attempted, what the substrate observed or refused, the vantage and directness of the observation, and how much of a declared population the claim covers. Ships as `vocabulary.yaml` with a crosswalk template, governance rules, and a validator that fails on any file it cannot parse.
