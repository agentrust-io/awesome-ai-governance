...
- [LEASH-8](https://github.com/Palo-Alto-AI-Research-Lab/agent-leash) - Practitioner control model for agents that already hold delegated authority: eight domains (identity, secrets, tool/skill supply chain, approvals, containment, egress, observability, incident response) scored as a 24-statement worksheet, plus a plan-vs-authorize pattern that keeps the authorization decision outside the model loop. MIT.
